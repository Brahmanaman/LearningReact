...
const FetchQR = () => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (postData, postId) => {
      queryClient.setQueriesData(["posts", pageNumber], (previousData) => {
        return previousData?.map((post) => {
          return post.id === postId ? { ...post, body: postData.data.body } : post;
        });
      });
    },
    onError: () => {
  ...
  return (
    <>
      <div className="post-container">
        ...
        <div className="dvbtn">
          <button onClick={() => setPageNumber((prev) => prev - 10)}>Prev</button>
          <h2>{pageNumber / 10 + 1}</h2>
          <button onClick={() => setPageNumber((prev) => prev + 10)}>Next</button>
        </div>
      </div>
    </>
  );
